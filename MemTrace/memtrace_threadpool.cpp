...
KNOB< std::string > KnobStatisticsOutputFile(KNOB_MODE_WRITEONCE, "pintool", "stat_file", "memtrace_threadpoolstats.out",
                                             "output file");
...
class ANALYSIS_CALL_INFO
{
  public:
    ...
    void InsertAnalysisCall(INT32 sizeofTraceInBuffer)
    {
        ...
        */
        INS_InsertCall(_ins, IPOINT_BEFORE, MAKE_AFUNPTR(APP_THREAD_REPRESENTITVE::RecordMEMREFInBuffer), IARG_FAST_ANALYSIS_CALL,
...
class TRACE_ANALYSIS_CALLS_NEEDED
{
  ...
  private:
    ...
    std::vector< ANALYSIS_CALL_INFO > _analysisCalls;
};
...
class BUFFER_LIST_MANAGER
{
  ...
  private:
    ...
    std::list< BUFFER_LIST_ELEMENT > _bufferList;
...
{
    DWORD status = WINDOWS::WaitForSingleObject(_freeBufferSem, 10000);
...
VOID APP_THREAD_REPRESENTITVE::ReturnFreeBuffer(VOID* buf, THREADID tid)
{
    ...
    WINDOWS::ReleaseSemaphore(_freeBufferSem, 1, NULL);
}
...
VOID APP_THREAD_REPRESENTITVE::AllocateFreeBuffers()
{
    ...
    _freeBufferSem = WINDOWS::CreateSemaphore(NULL, KnobNumBuffersPerAppThread - 1, 0x7fffffff, NULL);
}
...
VOID APP_THREAD_REPRESENTITVE::ReclaimFreeBuffers()
{
    ...
    WINDOWS::CloseHandle(_freeBufferSem);
    _freeBufferSem = NULL;
...
BUFFER_LIST_MANAGER::BUFFER_LIST_MANAGER(BOOL notifyExitRequired) : _exitEvent(NULL)
{
    ...
    _bufferSem = WINDOWS::CreateSemaphore(NULL, 0, 0x7fffffff, NULL);
    ASSERTX(_bufferSem != NULL);
    if (notifyExitRequired)
    {
        // Create manually reset event in non-signaled state.
        _exitEvent = WINDOWS::CreateEvent(NULL, TRUE, FALSE, NULL);
...
BUFFER_LIST_MANAGER::~BUFFER_LIST_MANAGER()
{
    if (_exitEvent != NULL)
    {
        WINDOWS::CloseHandle(_exitEvent);
    }
    WINDOWS::CloseHandle(_bufferSem);
}
...
BOOL BUFFER_LIST_MANAGER::PutBufferOnList(VOID* buf, VOID* endOfLastTraceInfBuffer,
                                          ...
                                          THREADID tid)
{
    if ((_exitEvent != NULL) && (WINDOWS::WaitForSingleObject(_exitEvent, 0) == WAIT_OBJECT_0))
    {
    ...
    BOOL success = WINDOWS::ReleaseSemaphore(_bufferSem, 1, NULL);
                                             ...
                                             THREADID tid)
{
    ...
    if (_exitEvent != NULL)
    {
        ...
        HANDLE handles[2] = {_bufferSem, _exitEvent};
        status            = WINDOWS::WaitForMultipleObjects(2, handles, FALSE, INFINITE);
    ...
    {
        status = WINDOWS::WaitForSingleObject(_bufferSem, INFINITE);
    }
    // Otherwise wait should exit due to semaphore.
    if (status != WAIT_OBJECT_0)
    {
        printf("  WAIT returned %lu, last error %lx\n", status, WINDOWS::GetLastError());
        fflush(stdout);
        ASSERTX(status == WAIT_OBJECT_0);
    }
...
{
    for (std::vector< ANALYSIS_CALL_INFO >::iterator c = _analysisCalls.begin(); c != _analysisCalls.end(); c++)
    {
...
VOID TraceAnalysisCalls(TRACE trace, VOID*)
{
    ...
    TRACE_InsertCall(trace, IPOINT_BEFORE, MAKE_AFUNPTR(APP_THREAD_REPRESENTITVE::AllocateSpaceForTraceInBuffer),
