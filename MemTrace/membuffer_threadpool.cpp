...
KNOB< std::string > KnobStatisticsOutputFile(KNOB_MODE_WRITEONCE, "pintool", "stat_file", "membuffer_threadpool_stats.out",
                                             "output file");
...
class BUFFER_LIST_MANAGER
{
  ...
  private:
    ...
    WINDOWS::HANDLE _exitEvent;
    WINDOWS::HANDLE _bufferSem;
                                          ...
                                          THREADID tid)
{
    ...
    WINDOWS::ReleaseSemaphore(_bufferSem, 1, NULL);
...
static VOID PrepareForFini(VOID* v)
{
    ...
    {
        printf("Waiting for exit of thread uid %p.\n", *it);
        fflush(stdout);
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    for (int i = 0; i < KnobNumProcessingThreads; i++)
    {
        ...
        printf("Created internal-thread uid=%p\n", threadUid);
        fflush(stdout);
