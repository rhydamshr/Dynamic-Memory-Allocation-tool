...
std::atomic_int numThreadsCalledWaitForAllThreadsStarted {0};
...
VOID APP_THREAD_REPRESENTITVE::ProcessBuffer(VOID* buf, UINT64 numElements, THREADID tid)
{
    ...
    if (numThreadsCalledWaitForAllThreadsStarted.load() == numThreadsInApp && maxThreadsSimultaneosulyInProcessBuffer < 2)
...
void CalledWaitForAllThreadsStarted(THREADID tid) { ++numThreadsCalledWaitForAllThreadsStarted; }
...
static void InstrumentRtn(RTN rtn, VOID*)
{
    ...
    {
        RTN_Open(rtn);
        RTN_InsertCall(rtn, IPOINT_BEFORE, MAKE_AFUNPTR(CalledWaitForAllThreadsStarted), IARG_THREAD_ID, IARG_END);
        RTN_Close(rtn);
